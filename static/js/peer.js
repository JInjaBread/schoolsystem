    ...
    .then(stream => {
        ...
        btnToggleAudio.onclick = function(){
            ...
            if(audioTracks[0].enabled){
                btnToggleAudio.className = 'fas fa-microphone';
                return;
            }

            btnToggleAudio.className = 'fas fa-microphone-slash';
        };

        btnToggleVideo.onclick = function(){
            videoTracks[0].enabled = !videoTracks[0].enabled;
            if(videoTracks[0].enabled){
                btnToggleVideo.className = 'fas fa-video';
                return;
            }

            btnToggleVideo.className = 'fas fa-video-slash';
        };
    })
    .then(e => {
        btnShareScreen.onclick = event => {
            ...
            navigator.mediaDevices.getDisplayMedia(constraints)
                .then(stream => {
                    ...
                    var localScreen = shareVideo('my-screen');
...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        remoteVideo = shareVideo(peerUsername + '-screen');
        setOnTrack(peer, remoteVideo);
        console.log('Remote video source: ', remoteVideo.srcObject);
...
function createAnswerer(offer, peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name){
    ...
    if(!localScreenSharing && !remoteScreenSharing){
        ...
        // set remote video
        var remoteVideo = shareVideo(peerUsername + '-screen');
        // and add tracks to remote video
        setOnTrack(peer, remoteVideo);
...
function createVideo(peerUsername){
    ...
    remoteVideo.style.width = 300 +"px";
    remoteVideo.style.height = 200 +"px";
...
function shareVideo(peerUsername){
    var videoContainer = document.querySelector('#screen-container');
    ...
    remoteVideo.autoplay = true;
    remoteVideo.playsinline = true;
    remoteVideo.style.width = 720 +"px";
    remoteVideo.style.height = 400 +"px";
    // btnPlayRemoteVideo.id = peerUsername + '-btn-play-remote-video';
    // btnPlayRemoteVideo.innerHTML = 'Click here if remote video does not play';
